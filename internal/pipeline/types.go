...
type Pipeline[A, B any] func(context.Context, A) dot.Result[B]
...
func Compose[A, B, C any](p1 Pipeline[A, B], p2 Pipeline[B, C]) Pipeline[A, C] {
	return func(ctx context.Context, a A) dot.Result[C] {
		// Check context before starting
		select {
		case <-ctx.Done():
			return dot.Err[C](ctx.Err())
		default:
		}
...
func Parallel[A, B any](pipelines []Pipeline[A, B]) Pipeline[A, []B] {
	return func(ctx context.Context, a A) dot.Result[[]B] {
		if len(pipelines) == 0 {
			return dot.Ok([]B{})
		}
		...
		results := make([]dot.Result[B], len(pipelines))
		...
		for _, r := range results {
			if !r.IsOk() {
				return dot.Err[[]B](r.UnwrapErr())
			}
			values = append(values, r.Unwrap())
		}

		return dot.Ok(values)
	}
}
...
func FlatMap[A, B any](fn func(A) dot.Result[B]) Pipeline[A, B] {
	return func(ctx context.Context, a A) dot.Result[B] {
		select {
		case <-ctx.Done():
			return dot.Err[B](ctx.Err())
...
func Filter[A any](pred func(A) bool) Pipeline[A, A] {
	return func(ctx context.Context, a A) dot.Result[A] {
		select {
		case <-ctx.Done():
			return dot.Err[A](ctx.Err())
		default:
			if pred(a) {
				return dot.Ok(a)
			}
			return dot.Err[A](errors.New("value filtered out by predicate"))
		}
	}
}
